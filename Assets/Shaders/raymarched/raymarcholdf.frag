...
float r_maxdist = 10.0f;
int r_steps = 48;
int r_bounces = 2;
bool doReflections = true;

float i_maxdist = 20.0f;
int i_steps = 32;
int i_samples = 1;
...
void setupMaterialsAdvanced(vec3 p)
{
    ...
    materials[3].reflectionFactor = 0.0 + 0.9 * mod(floor(p.x) + floor(p.z), 2.0);
...
vec4 sceneSDF(vec3 p ) // scene
{
    ...
    for (int i = 0; i < lSDFS.length(); i++ ){
        ...
        vec3 gp = transformP(p, lSDFS[i].globalTransform);
        vec3 fp = gp - lSDFS[i].position.rgb;
        vec3 nsdfv = texture3DSDFUV(fp, lSDFS[i].extents.rgb, lSDFS[i].SDF_Handle, 0);
        
        if (i <= 0) {
            //            tsdf = vec4(nsdfv.r, materialID3, nsdfv.gb);
            tsdf = vec4(nsdfv.r, i, nsdfv.gb);
            continue;
        }
        //        tsdf = fOpUnionRoundID4(tsdf,vec4(nsdfv.r, materialID3, nsdfv.gb), 1.0); //fOpUnionRoundID4 fOpUnionID4
        tsdf = fOpUnionID4(tsdf,vec4(nsdfv.r, i, nsdfv.gb)); //fOpUnionRoundID4 fOpUnionID4
        
    }
    return vec4(tsdf);
    
    vec3 sdfv = texture3DSDFUV(p, vec3(1.0), Susanne64_handle, 0);
...
float calcAO( in vec3 pos, in vec3 nor, in int samples )
{
    float ao = 0.0;
    for( int i=0; i<samples; i++ )
    {
        vec3 ap = forwardSF( float(i), float(samples) );
        float h = hash1(float(i));
        ap *= sign( dot(ap,nor) ) * h*0.1;
        ao += clamp( sceneSDF( pos + nor*0.01 + ap ).x*3.0 , 0.0, 1.0 );
    }
    ao /= float(samples);
...
void main()
{
    ...
    for (int i = 0; i < SSAAsamples; i++) // 4 samples
    {
        ...
        //dbg += vec3(inversesqrt(max(firsthit.lowestDistance, 0.0)) * 0.02);
        //dbg += vec3(inversesqrt(max(firsthit.distance, 0.0)) * 0.02);
        //dbg += vec3(inversesqrt(abs(firsthit.lowestDistance)) * 0.01);
       // if (firsthit.isHit) dbg = vec3(1.0, 1.0, 1.0);
        //dbg += vec3(vec3(inversesqrt(float(firsthit.isHit))) * vec3(inversesqrt(firsthit.lowestDistance)))  * 0.01;
        //dbg = vec3(firsthit.lowestDistance) * 0.1;
        
        hitcount++;
        //texture_diffuse_Handle
        //vec3 albedot = textureLod( sampler2D(texture_diffuse_Handle), firsthit.uv, 0).rgb;
        vec3 albedot = textureLod( sampler2D(lSDFS[int(firsthit.materialID)].texture_diffuse_Handle), firsthit.uv, 3).rgb;
        ...
        ngbuffer.albedo += albedot;
        //ngbuffer.albedo += vec3(firsthit.uv, 0.0);
        //ngbuffer.albedo += calcAO(firsthit.hitpos, firsthit.normal, 32);
        //ngbuffer.albedo += nngbuffer.albedo;
        //ngbuffer.normal += firsthit.normal;
        //ngbuffer.depth += nngbuffer.depth;
        
        if (firsthit.isHit) anyhit = true;
    ...
    vec3 colour = vec3(0.0);
    //colour = vec3(dbg / hitcount);

    
    colour = ngbuffer.albedo / hitcount;
    //colour = ngbuffer.normal / hitcount;
    
    //if (hitcount == 0 || !anyhit) colour = ngbuffer.albedo / hitcount;
    //if (hitcount == 0) colour = ngbuffer.albedo / hitcount;
