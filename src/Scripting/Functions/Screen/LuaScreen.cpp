...
void luaScreen::SetScreen(sol::state& luaState) {
	luaState["SetWindow"] = [](std::string windowVariableType, std::string value) {
		...
		if (windowVariableType == "Title") windowHandler::s_WindowTitle = value; // Title
			...
			if (value != "true" && value != "false") { std::cout << "[CPP] Exemption Lua SetWindow: 'Vysnc' can only be 'true' or 'false' But value is: " << value << std::endl; } //debug
		}
		else std::cout << "[CPP] Exemption Lua SetWindow: Unsupported 'windowVariableType' known as: " << windowVariableType << std::endl; // debug
		};
}